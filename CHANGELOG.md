...
- Fix the artifact `npm publish` call added in #6: a bare `artifact/herdwatch-sentry-cordova-*.tgz` path is ambiguous to npm's argument parser and got resolved as a `github:artifact/...` shorthand instead of a local file, so it tried to `git ls-remote` a nonexistent repo instead of publishing. Prefixing it with `./` forces npm to treat it as a file path. ([#7](https://github.com/herdwatch-apps/sentry-cordova/pull/7))
